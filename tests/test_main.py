...
def test_bubble_sort_orders_numbers(sample_numbers: list[int]) -> None:
    result = main.bubble_sort(sample_numbers, delay=0, show_steps=False)
    assert result == [1, 2, 4, 5, 8]


def test_bubble_sort_handles_empty_and_singleton_lists() -> None:
    assert main.bubble_sort([], delay=0, show_steps=False) == []
    assert main.bubble_sort([7], delay=0, show_steps=False) == [7]
...
def test_bubble_sort_keeps_in_place_behavior() -> None:
    ...
    result = main.bubble_sort(values, delay=0, show_steps=False)
...
def test_bubble_sort_does_not_sleep_when_delay_is_zero(
    ...
    main.bubble_sort([4, 1, 3], delay=0, show_steps=True)
...
def test_bubble_sort_can_disable_step_output(
    ...
    main.bubble_sort([4, 2, 1], delay=0, show_steps=False)
